...
| `/srv/seedbox/data` | `/srv/seedbox/data` | `temp/`, `complete/` et `media/`, en **lecture-écriture** |
...
!!! tip "Le disque n'est PAS monté en entier — c'est délibéré"
    ...
    `data/` et `watch/`. La session rTorrent et la configuration ruTorrent
...
4. Chemin du backend local : `/srv/seedbox` — tu n'y verras que `data/` et `watch/`.
